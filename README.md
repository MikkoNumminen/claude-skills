...
| [`skills-freshness`](skills/skills-freshness/SKILL.md)                      | Audit skills for staleness without reading the ones that haven't changed. A pure-stdlib Python script computes sha256 over every file in each skill directory and compares against a stored manifest (project + global scopes, separate manifests each). Only new/changed/removed skills are surfaced; unchanged skills are never loaded into context. Skills can opt in to declared TOML checks (`path_exists`, `file_contains`, `file_lacks`, `no_broken_md_links`, `command_exists`) under `# Freshness check` in their SKILL.md; otherwise the LLM does a generic frontmatter + description + broken-ref pass. Asks before editing. Stdlib-only tests under `tests/`. | shipped   |
