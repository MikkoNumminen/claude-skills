...
| [`skills-freshness`](skills/skills-freshness/SKILL.md)                      | Audit installed skills for staleness without re-reading the ones that haven't changed. A pure-stdlib Python script computes sha256 over each skill directory and compares against a stored manifest (project + global scopes, separate manifests). Only new / changed / removed skills are surfaced; unchanged skills never enter LLM context. Skills can opt in to declared TOML checks under `# Freshness check`; otherwise the LLM does a generic frontmatter + description + broken-ref pass. Stdlib-only test suite. | shipped   |
