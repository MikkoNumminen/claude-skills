...
This repo started life as `claude-audit-skill` housing a single audit skill. It now hosts a small family of skills — read-only audits, a handful of cross-cutting measurement / freshness / quality tools, and a few personal-namespace helpers.
...
| [`dotnet-audit`](skills/dotnet-audit/SKILL.md)                              | Read-only audit for ASP.NET Core / EF Core / C# anti-patterns that generic audits miss (captive DI dependencies, untracked-vs-tracked EF queries, missing CancellationToken, DbContext concurrency, sync-over-async, `@Html.Raw` XSS, overposting, open redirects, swallowed exceptions incl. cancellation). Five parallel sub-agents over non-overlapping .NET scopes; each check pairs a smell with a legitimate counter-example. Runs a pre-flight fit check and aborts cleanly if the codebase isn't .NET. | shipped   |
...
| [`llm-injection-audit`](skills/llm-injection-audit/SKILL.md)                | Read-only audit for prompt-injection defense **at the LLM boundary** (any language). Maps every untrusted-text→prompt splice, then grades six defense-in-depth layers generic audits miss — data/instruction separation, input-symptom flagging, output-authority bounding, structural grounding, a deterministic trust anchor, and a red-team regression fixture. Each check pairs a smell with a legitimate counter-example, in C# **and** Python. Pre-flight aborts if the codebase calls no LLM. States plainly that injection is unsolved — it audits for layers, never proves safety. | shipped   |
...
| [`skills-quality`](skills/skills-quality/SKILL.md)                          | Token-economy audit. Deterministic pre-pass flags smells (long imperative prose with no companion script, missing frontmatter, oversized SKILL.md, excessive code blocks). LLM review fires only for flagged or changed skills. Manifest key is (skill_hash + ruleset_hash), so editing the rules re-enters every skill for review. Shares change-detection with `skills-freshness` via `skills/_lib/`. | shipped   |
...
| [`mikko-skills`](skills/mikko-skills/SKILL.md)                              | Personal helper: lists every installed `mikko-*` skill as a plain-English one-liner (reads each skill's `barney:` frontmatter field, falls back to `description`). One-screen output for "what mikko skills do I have" / onboarding a machine; for dense per-skill detail use `mikko-help` instead. | shipped   |
...
**Length guideline for `barney`:** target around 140 characters so the table layout in `/mikko-help --barney` stays clean on a typical terminal width. Longer lines are fine in the SKILL.md itself; `mikko-help` truncates with an ellipsis when rendering the table. For comparison, the longest barney lines in this library today are ~240 chars (ai-codegen-smell-audit) and ~200 chars (security-audit) — readable in full when wrapped, but cropped in the listing.
