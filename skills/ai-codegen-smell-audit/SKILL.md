...
description: Read-only audit for failure modes recurring in LLM-generated code — defensive guards on impossible cases, generic names, swallowed errors, single-use helpers, mirror tests. Each check pairs a smell with a legitimate counter-example, so findings are signal not vibes; writes severity-ranked results to `docs/audits/ai-smell-<date>.md`. Use for "smell audit", "check for AI-codegen smells", "review this branch for LLM sludge", or a pass before merging a large generated diff. Every finding is a specific testable pattern, not a witch hunt for "AI-written code".
...
- **Workflow:** `.github/workflows/codegen-smell-audit.yml` (AudiobookMaker — not present in this library)
- **Script:** `scripts/check_codegen_smells.py` (AudiobookMaker — not present in this library)
...
See [PROVENANCE.md](PROVENANCE.md) for the original calibration run against AudiobookMaker (2026-05-17) and the second-run verification.
