...
description: Read-only audit for six React anti-patterns in component code (key=index, useEffect for derived state, dep-array lies, state mutation, missing effect cleanup, multiple sources of truth). Each check pairs a smell with a legitimate counter-example. Runs a pre-flight fit check first — aborts cleanly if the codebase isn't React. Use for "audit my React code", "review this for React anti-patterns", "check my hooks", "find re-render issues", or before merging a substantial component PR. Every finding is a specific testable pattern, not a non-idiomatic-React witch hunt.
