...
description: Detect drift between what the repo contains and what `README.md` claims, then rewrite only the drifted sections in the README's existing voice across five axes (file structure, dependencies, skills, features, status). Extracts a voice profile first so edits read like the original author. Write-with-approval — emits a working-tree README edit plus a drift report, then STOPS (no commit, no push, no full regeneration). Use for "sync the README", "update the README", "check README drift", or after a release-cut. NOT for initial README creation, full rewrites, or CONTRIBUTING/CHANGELOG edits.
